...
export class FileBrowser {
	...
	constructor(container: HTMLElement, initialPath: string = '/') {
		...
		this.container.classList.add('flmngr-container')
		...
		filesContainer.className = 'flmngr-files-container'
	...
	/**
	 * Отрисовывает базовую структуру UI
	 */
	private renderUI(): void {
		this.container.innerHTML = `
			<!-- Header с поиском и действиями будет добавлен через Toolbar -->
			
			<div class="flmngr-content">
				<!-- Sidebar будет добавлен через Sidebar -->
				<!-- Files будут добавлены через FileList -->
			</div>
			
			<div class="flmngr-loading">
				<div class="flmngr-loading-spinner"></div>
				<div class="flmngr-loading-text">Загрузка...</div>
			</div>
			
			<div class="flmngr-footer">
				<span class="flmngr-footer-path">Путь: /</span>
				<div class="flmngr-disk-space">
					<div class="flmngr-disk-space-bar">
						<div class="flmngr-disk-space-used" style="width: 0%"></div>
					</div>
					<div class="flmngr-disk-space-info">
	...
	private loadDirectory(path: string): void {
		...
		const pathElement = this.footerElement.querySelector('.flmngr-footer-path')
		if (pathElement) {
	...
	private showLoading(show: boolean): void {
		...
		const filesElement = this.container.querySelector('.flmngr-files')
		if (filesElement) {
			filesElement.classList.toggle('hidden', show)
		}
	}
